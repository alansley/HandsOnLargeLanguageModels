...
# Note: This takes a minute or so! Use `workers=1` AND `seed=<SOME_SPECIFIC_VALUE>` to get reproducible results!
...
print(f"\nTop {num_similar_songs} similar songs to: {chosen_song_id} ({chosen_song_details}):")
print(f"{'ID':<6} | {'Artist':<20} | {'Title':<30} | {'Similarity':>10}")
print("-" * 76)

for similar_song_id, similar_song_score in most_similar_songs:
    details = get_song_details(int(similar_song_id))
    # Example format: "id: 2849 - artist: Iron Maiden - title: Run To The Hills"
    parts = details.split(' - ')
